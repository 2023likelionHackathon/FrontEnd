...
const Loc_with_sijang = () => {
    ...
    let GwaingJin = <div className={styles.GwaingJin}>
        아직 등록된 시장이 없습니다.
    </div>
    let GangBuk = <div className={styles.GangBuk}>
        아직 등록된 시장이 없습니다.
    </div>
    let SeongDong = <div className={styles.SeongDong}>
    ...
    const loc1ClassName = selectedLocation === '광진구' ? `${styles.loc} ${styles.loc1} ${styles.selected}` : `${styles.loc} ${styles.loc1}`;
    const loc2ClassName = selectedLocation === '강북구' ? `${styles.loc} ${styles.loc2} ${styles.selected}` : `${styles.loc} ${styles.loc2}`;
    const loc3ClassName = selectedLocation === '성동구' ? `${styles.loc} ${styles.loc3} ${styles.selected}` : `${styles.loc} ${styles.loc3}`;


    return (
        <div className={styles.container}>
            <div className={styles.nav_bar}>
                <div className={loc1ClassName} onClick={() => handleLocationClick('광진구')}>
                    광진구
                </div>
                <div className={loc2ClassName} onClick={() => handleLocationClick('강북구')}>
                    강북구
                </div>
                <div className={loc3ClassName} onClick={() => handleLocationClick('성동구')}>
                    성동구
                </div>
            </div>
            <div className={`${styles.sijang_name}`}>
                {sijangContent}
            </div>
        </div>
    );
